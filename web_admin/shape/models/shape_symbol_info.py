...
class ShapeSymbolInfoModel(BaseModel):
    """"""
    ...
    def get_all(self, trade_day):
        ...
        if trade_day:
            query_set = ShapeSymbolInfoModel.objects.filter(trade_date=trade_day)
        else:
            query_set = ShapeSymbolInfoModel.objects.filter(trade_date=date)
        return query_set