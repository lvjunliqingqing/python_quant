...
class StockOpenSymbolDataModel(BaseModel):
    """股票加入交易模型类"""
    ...
    class Meta:
        db_table = 'js_stock_open_symbol_data'
    ...
    def get_shape_not_open(self, trade_day):
        ...
        try:
            if trade_day[0]:
                obj = StockOpenSymbolDataModel.objects.filter(trade_date=trade_day[0])
            else:
                obj = StockOpenSymbolDataModel.objects.filter(trade_date=date)
            if obj:
                for i in obj:
                    if i.exchange != "CZCE":
    ...
    def get_all(self, trade_day, audit_status, account_id):
        date = datetime.now().strftime("%Y-%m-%d")
        if trade_day:
            query_set = StockOpenSymbolDataModel.objects.filter(Q(trade_date=trade_day) & Q(audit_status=audit_status) & Q(account_id=account_id))
        else:
            query_set = StockOpenSymbolDataModel.objects.filter(Q(trade_date=date) & Q(audit_status=audit_status) & Q(account_id=account_id))
        return query_set
