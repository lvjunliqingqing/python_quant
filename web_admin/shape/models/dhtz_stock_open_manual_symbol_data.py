...
class DhtzStockOpenManualSymbolDataModel(BaseModel):
    """"""
    ...
    class Meta:
        db_table = 'js_stock_open_manual_symbol_data'
        verbose_name = '手动加入交易的数据表(手动交易清单表)'
    ...
    def query_by_audit_status_account_id_status(self, account_id, audit_status, status):
        if audit_status == "all" and status == "all":
            query_set = DhtzStockOpenManualSymbolDataModel.objects.filter(account_id=account_id)
        elif audit_status == "all":
            query_set = DhtzStockOpenManualSymbolDataModel.objects.filter(account_id=account_id, status=status)
        elif status == "all":
            query_set = DhtzStockOpenManualSymbolDataModel.objects.filter(account_id=account_id, audit_status=audit_status)
        else:
            query_set = DhtzStockOpenManualSymbolDataModel.objects.filter(
                Q(status=status)
