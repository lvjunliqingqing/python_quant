...
def audit_manual_info(request):
    if request.method == "POST":
        ...
        query_set = DhtzOpenManualSymbolDataModel().query_by_audit_status_account_id_status(account_id, audit_status, status)
        desc_map = DhtzStrategyDesc().desc_map()
        if query_set:
            for i in query_set:
                symbol = re.match(r'^[a-zA-Z]+', i.symbol).group().upper()
                key = f"{i.strategy_name}.{symbol}.{i.exchange}"
                dhtz_open_symbol_data_list.append(
                    {
                        "id": i.id,
                        "account_id": i.account_id,
                        "symbol": f"{i.symbol}.{i.exchange}",
                        "exchange": i.exchange,
