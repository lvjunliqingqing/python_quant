<template>
  <div class="app-container">
    ...
    <el-row style="margin:10px;">
      ...
      <el-select v-model="audit_status" placeholder="状态" @change="changeSelect">
        <el-option
    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <span v-if="col.key != 'direction' && col.key != 'audit_status'">{{ scope.row[col.key] }}</span>
        </template>
...
export default {
  ...
  created() {
    ...
    this.dynamicValidateForm.audit_status = 0
  },
  methods: {
    changePicker(val) {
      this.trade_day = val
    ...
    loadData() {
      this.$store.dispatch('audit/info', this.dynamicValidateForm).then((data) => {
        var colums = data.col
    ...
    AuditClick(row, index) {
      ...
      if (this.audit_data.audit_status) {
        this.$message({
    ...
    submitForm(audit_status) {
      ...
      this.$store.dispatch('audit/save', this.audit_data).then((data) => {
        console.log(data)
