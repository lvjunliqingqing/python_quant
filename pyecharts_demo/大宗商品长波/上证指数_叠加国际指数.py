...
class SZEVENTBACK(object):
    ...
    def create_sz_line_with_multiple_axis(self):
        sz_line = Line(init_opts=opts.InitOpts(theme='light',
                                               width='1600px',
                                               height='800px'))
        sz_line.add_xaxis(data["times"])
        ...
        # 分别指定使用那个Y轴
        sz_line.add_yaxis('上证指数', data["close"], yaxis_index=0, is_smooth=True, label_opts=opts.LabelOpts(is_show=False))

        sz_line.set_global_opts(
