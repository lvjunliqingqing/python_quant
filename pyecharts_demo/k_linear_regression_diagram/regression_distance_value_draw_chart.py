...
class RegressionDistanceValueDrawChart(object):

    def __init__(self):
        super(RegressionDistanceValueDrawChart, self).__init__()
    ...
    def draw_chart(self, df, security):
        ...
        self.grid_chart(overlap_kline_line, regression_distance_curve, security)

    def grid_chart(self, overlap_kline_line, regression_distance_curve, security):
        # 创建网格图形对象
        ...
        grid_chart.render(f"html_chart/{security}_k线线性回归图.html")


if __name__ == "__main__":
    df = pd.read_csv("ZN9999.XSGE.csv")
    security = df["security"].to_list()[0]
    RegressionDistanceValueDrawChart().draw_chart(df, security)
