...
class CommodityIndex(object):
    ...
    def draw_chart(self):
        kline = self.create_k_line(data["times"], data["datas"], "价格回归距离曲线")
        ma5_line = self.create_k_line_line(data["times"], "MA5", calculate_ma(day_count=5))
        ma10_line = self.create_k_line_line(data["times"], "MA10", calculate_ma(day_count=10))
        ma20_line = self.create_k_line_line(data["times"], "MA20", calculate_ma(day_count=20))
        # k线图区元素对象(Kline + Line)
        ...
        # 价格回归曲线图形数据元素对象
        regression_distance_curve = self.create_regression_distance_curve(data["times"], data["difs"], "收盘价到回归曲线的垂直距离")
    ...
    def grid_chart(self, overlap_kline_line, regression_distance_curve):
        ...
        grid_chart.render("假数据_k线线性回归图.html")
