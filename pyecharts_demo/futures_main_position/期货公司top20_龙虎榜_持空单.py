...
if __name__ == "__main__":
    ...
    for security in main_price_relationship.main_all_security:
        ...
        sell_position_list = sell_volume_series.to_list()
        main_times = sell_main_position_df.groupby(["trade_date"])["trade_date"].first().to_list()
        print(main_times)
        ...
        if not m_p_d_df.empty:
            data = {
                    ...
                    "main_position": {
                        "times": main_times,
                        "buy_main_open_interest": sell_position_list
                    },
                    }

            series_name = "期货公司top20主力持空单"
            filename = f"html_chart/期货公司top20_龙虎榜持空单/{data['security']}期货公司top20_龙虎榜持空单与股价的关系.html"
            title = f"{data['security']}期货公司top20_龙虎榜持空单与股价的关系"
