...
from vnpy.trader.utility import load_json, save_json, save_parameter_optimization_data_to_csv, get_letter_from_symbol
from ..engine import (
    APP_NAME,
    EVENT_SPREAD_BACKTESTER_LOG,
    EVENT_SPREAD_BACKTESTER_BACKTESTING_FINISHED,
    EVENT_SPREAD_BACKTESTER_OPTIMIZATION_FINISHED,
    OptimizationSetting, SpreadBacktesterEngine
)
from ..spread_chart.item import SpreadCandleItem
from ...utility import TerminateThread, RelationshipChartDialog


class SpreadBacktesterManager(QtWidgets.QWidget):
    """"""

    setting_filename = "spread_backtester_setting.json"
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        self.spread_backtester_engine = main_engine.get_engine(APP_NAME)
        self.class_names = []
        self.settings = {}
        ...
        self.init_ui()
        self.register_event()
        self.spread_backtester_engine.init_engine()
        self.init_strategy_settings()
        self.load_backtesting_setting()

    def init_strategy_settings(self):
        """"""
        self.class_names = self.spread_backtester_engine.get_strategy_class_names()

        for class_name in self.class_names:
            setting = self.spread_backtester_engine.get_default_setting(class_name)
    ...
    def init_ui(self):
        """"""
        self.setWindowTitle("套利回测")

        # Setting Part
        self.class_combo = QtWidgets.QComboBox()

        self.symbol_line = QtWidgets.QLineEdit("AL9999.XSGE")
        # 回测时,输入本地代码后,按回车键自动设置价格跳动和合约乘数
        self.symbol_line.returnPressed.connect(self.set_vt_symbol)

        self.add_spread_button = QtWidgets.QPushButton("价差创建")
        self.add_spread_button.clicked.connect(self.spread_create)
        ...
        form.addRow("价差", self.add_spread_button)
    ...
    def spread_create(self):
        """"""
        dialog = SpreadDataDialog(self.spread_backtester_engine)
        dialog.exec_()
    ...
    def register_event(self):
        ...
        self.event_engine.register(EVENT_SPREAD_BACKTESTER_LOG, self.signal_log.emit)
        self.event_engine.register(
            EVENT_SPREAD_BACKTESTER_BACKTESTING_FINISHED, self.signal_backtesting_finished.emit)
        self.event_engine.register(
            EVENT_SPREAD_BACKTESTER_OPTIMIZATION_FINISHED, self.signal_optimization_finished.emit)
    ...
    def process_backtesting_finished_event(self, event: Event):
        """"""
        statistics = self.spread_backtester_engine.get_result_statistics()
        self.statistics_monitor.set_data(statistics)

        df = self.spread_backtester_engine.get_result_df()
    ...
    def start_backtesting(self):
        """"""
        if not self.spread_backtester_engine.spread:
            QtWidgets.QMessageBox.critical(self, "价差回测提示", "请创建好价差后再进行回测")
            return
        spread = self.spread_backtester_engine.spread
        ...
        result = self.spread_backtester_engine.start_backtesting(
            class_name,
            spread,
            interval,
            start,
    ...
    def stop_backtesting(self):
        ...
        self.spread_backtester_engine.thread = None
    ...
    def start_optimization(self):
        ...
        self.linear_params = optimization_setting.params  # 获取参数优化对话框里面的数据
        self.target_display = dialog.target_display

        self.spread_backtester_engine.start_optimization(
            spread,
            class_name,
            vt_symbol,
            interval,
    ...
    def show_optimization_result(self):
        """"""
        result_values = self.spread_backtester_engine.get_result_values()
    ...
    def show_backtesting_trades(self):
        ...
        if not self.trade_dialog.is_updated():
            trades = self.spread_backtester_engine.get_all_trades()
    ...
    def show_backtesting_orders(self):
        ...
        if not self.order_dialog.is_updated():
            orders = self.spread_backtester_engine.get_all_orders()
    ...
    def show_daily_results(self):
        ...
        if not self.daily_dialog.is_updated():
            results = self.spread_backtester_engine.get_all_daily_results()
            self.daily_dialog.update_data(results)

            # # 输出最小开仓数量和最开仓数量
            # end_pos_data = [abs(int(obj.end_pos)) for obj in results]
            # print(min(end_pos_data), max(end_pos_data))
        self.daily_dialog.exec_()

    def show_candle_chart(self):
        """"""
        if not self.candle_dialog.is_updated():
            history = self.spread_backtester_engine.get_history_data()
    ...
    def edit_strategy_code(self):
        """"""
        class_name = self.class_combo.currentText()
        file_path = self.spread_backtester_engine.get_strategy_class_file(class_name)
    ...
    def reload_strategy_class(self):
        """"""
        self.spread_backtester_engine.reload_strategy_class()
...
class OptimizationResultMonitor(QtWidgets.QDialog):
    ...
    def __init__(
        self, result_values: list, target_display: str, linear_params: dict
    ):
        """"""
        super().__init__()
    ...
    def init_ui(self):
        ...
        if self.result_values[0][2]:
            ...
            self.table_head = ["参数", self.target_display, "年化收益率", "夏普比率", "胜率", "最大回撤比"]
        ...
        for n, tp in enumerate(self.result_values):
            ...
            target_cell = QtWidgets.QTableWidgetItem(str(f'{target_value:,.2f}'))
            ...
            if _:
                ...
                # table.setItem(n, 6, win_ratio)
                # table.setItem(n, 7, win_loss_ratio)
                # table.setItem(n, 8, return_risk_ratio)
                # table.setItem(n, 9, real_yields)

        # Create layout
        button = QtWidgets.QPushButton("保存")
        button.clicked.connect(self.save_csv)

        relationship_chart_button = QtWidgets.QPushButton("参数优化-线性关系图表")
        relationship_chart_button.clicked.connect(self.show_relationship_chart)

        hbox = QtWidgets.QHBoxLayout()
        hbox.addWidget(relationship_chart_button)
        hbox.addStretch()
    ...
    def show_relationship_chart(self): # noqa

        relationship_chart_dialog = RelationshipChartDialog(self.result_values, self.linear_params, self.target_display)
        relationship_chart_dialog.exec_()
...
class SpreadDataDialog(QtWidgets.QDialog):
    ...
    def __init__(self, spread_backtester_engine: SpreadBacktesterEngine):
        """"""
        super().__init__()
        self.spread_filename = "spread_backtester_spread_data.json"
        self.spread_backtester_engine: SpreadBacktesterEngine = spread_backtester_engine
    ...
    def init_ui(self):
        ...
        button_add = QtWidgets.QPushButton("创建~价差")
        button_add.clicked.connect(self.create_spread)
        ...
        self.setLayout(grid)
        self.set_grid_parameter()

    def set_grid_parameter(self):
        """
        通过读取json文件中价差配置数据,写入到价差配置对话中。避免每次都要手动输入。
        """
        leg_from_json_settings = load_json(self.spread_filename)
        if not leg_from_json_settings:
            return

        self.name_line.setText(leg_from_json_settings["spread_name"])
        self.active_line.setText(leg_from_json_settings["active_symbol"])
        self.min_volume_combo.setCurrentText(str(leg_from_json_settings["min_volume"]))
        num_temp = len(leg_from_json_settings["spread"])

        for i, d in enumerate(self.leg_widgets):
            if i >= num_temp:
                return
            d["symbol"].setText(leg_from_json_settings["spread"][str(i)]["vt_symbol"])
            d["price"].setText(str(leg_from_json_settings["spread"][str(i)]["price_multiplier"]))
            d["trading"].setText(str(leg_from_json_settings["spread"][str(i)]["trading_multiplier"]))

            if not leg_from_json_settings["spread"][str(i)]["inverse_contract"]:
                d["inverse"].setCurrentIndex(0)
            else:
                d["inverse"].setCurrentIndex(1)
    ...
    def create_spread(self):
        ...
        active_symbol = self.active_line.text()  # 主动腿代码
        ...
        for i, d in enumerate(self.leg_widgets):
            try:
                ...
                leg_settings[vt_symbol] = data
                ix_legData_map[str(i)] = data
            except ValueError:
                pass
        ...
        if active_symbol not in leg_settings:
            ...
            return
        self.spread_backtester_engine.create_spread(
            spread_name,
        ...
        legData_setting = { # noqa
            "spread_name": spread_name,
            "active_symbol": active_symbol,
            "min_volume": min_volume,
            "spread": ix_legData_map

        }
        save_json(self.spread_filename, legData_setting)
...
class CandleChartDialog(QtWidgets.QDialog):
    ...
    def init_ui(self):
        ...
        self.chart.add_item(SpreadCandleItem, "candle", "candle")
