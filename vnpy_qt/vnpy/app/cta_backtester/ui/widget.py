...
from .backtest_chart_dialog import NewCandleChartDialog
from ..engine import (
    APP_NAME,
...
class BacktesterManager(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.candle_dialog = NewCandleChartDialog()
    ...
    def show_daily_results(self):
        ...
        if not self.daily_dialog.is_updated():
            ...
            results.reverse()
            for obj in results:
                end_pos_data.append(abs(int(obj.end_pos)))
    ...
    def show_candle_chart(self):
        ...
        if not self.candle_dialog.is_updated():
            ...
            if self.candle_dialog.chart.trade_connection_line_hidden_flag:  # 决定是否显示交易盈亏连接曲线
                for item in self.candle_dialog.items:
                    item.hide()

            self.idx_order_map()
            self.idx_trade_map()
            self.candle_dialog.chart.add_trades(self.trades)
            self.candle_dialog.chart.add_orders(self.orders)

            # 把资金曲线设置到k线图组件上
            df = self.backtester_engine.get_result_df()
            if hasattr(df, "balance"):
                self.candle_dialog.update_balance(df["balance"])
            if self.candle_dialog.chart.net_asset_curve_hidden_flag:  # 决定是否显示资金曲线。
                self.candle_dialog.candle_left_item.hide()

            # 把回测指标统计结果设置到k线图组件中
            result_statistics = self.backtester_engine.get_result_statistics()
            self.candle_dialog.update_backester_result(result_statistics)

            # 把成交记录以创建按钮的形式设置到k线图组件中
            trades_record = self.backtester_engine.get_trades_record()  # 获取策略中特别处理过的成交记录数据
            if trades_record:
                self.candle_dialog.update_trades_record(trades_record)
    ...
    def idx_order_map(self):
        """idx和order的映射"""
        orders = self.backtester_engine.get_all_orders()
        for order in orders:
            idx = self.candle_dialog.dt_ix_map[order.datetime]
            if idx in self.orders:
                self.orders[idx][order.orderid] = order
            else:
                self.orders[idx] = {order.orderid: order}

    def idx_trade_map(self):
        """idx和trade的映射"""
        trades = self.backtester_engine.get_all_trades()
        for trade in trades:
            idx = self.candle_dialog.dt_ix_map[trade.datetime]
