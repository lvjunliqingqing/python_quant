...
from vnpy.trader.constant import (Direction, Offset, Exchange,
                                  Interval, Status)
from vnpy.trader.object import TradeData, BarData, TickData, OrderData
...
class BacktestingEngine:
    ...
    def run_optimization(self, spread, optimization_setting: OptimizationSetting, output=True):
        """"""
    ...
    def run_ga_optimization(self, spread, optimization_setting: OptimizationSetting, population_size=100, ngen_size=30, output=True):
        """"""
        # Get optimization setting and target
    ...
    def cross_algo(self):
        ...
        for algo in list(self.active_algos.values()):
            ...
            if algo.status == Status.SUBMITTING:
                algo.status = Status.NOTTRADED
    ...
    def get_all_orders(self):
        ...
        """
        for algoid, algo in self.algos.items():  # 通过self.algos来最终绝对order的状态和成交数量
            self.limit_orders[f"{self.gateway_name}.{algoid}"].status = algo.status
            self.limit_orders[f"{self.gateway_name}.{algoid}"].traded = algo.traded
