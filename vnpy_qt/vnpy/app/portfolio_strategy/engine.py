...
from vnpy.trader.utility import load_json, save_json, extract_vt_symbol, round_to, BISTRATEGYNAME
...
class StrategyEngine(BaseEngine):
    ...
    def load_bars(self, strategy: StrategyTemplate, days: int, interval: Interval, use_database: bool):
        """"""
        ...
        for vt_symbol in vt_symbols:
            data = self.load_bar(vt_symbol, days, interval, use_database)
    ...
    def load_bar(self, vt_symbol: str, days: int, interval: Interval, use_database: bool) -> List[BarData]:
        """"""
        ...
        # 前面的方式获取不到数据时，再从数据中查询。
        if not data:
            exchange = Convert().convert_jqdata_exchange(exchange.value)
    ...
    def add_strategy(
        ...
        # 添加策略时把相关的策略数据添加到数据库中(数据库中不存在才保存)
        account_id = self.get_account_id()
        portfolio = PortfolioRamValue()
        value = portfolio.search_value_plotfolio(strategy_name, account_id)
        if not value:
            risk_ctrl_cond = strategy.risk_ctrl_cond
            sql = portfolio.insert_value_plotfolio(class_name, str(vt_symbols), strategy_name, risk_ctrl_cond, account_id)
            if not sql:
                self.write_log(f"创建策略失败，变量插入数据库失败")
                return
        else:
            self.write_log(f"策略名已存在{strategy_name}")
            strategy.risk_ctrl_cond = json.loads(value.risk_ctrl_cond)  # 数据库中存在时就把查询的到结果赋值给策略
    ...
    def get_account_id(self):
        account_id = self.main_engine.get_gateway('CTP').td_api.userid  # 获取账号 account_id
        if not account_id:
            SETTING_FILENAME = "connect_ctp.json"
            config_con = load_json(SETTING_FILENAME)
            account_id = config_con["用户名"]
            self.write_log('请先登陆账户再重启')
        return account_id
    ...
    def _init_strategy(self, strategy_name: str):
        ...
        for vt_symbol in strategy.vt_symbols:
            strategies = self.symbol_strategy_map[vt_symbol]
            strategies.append(strategy)
            self.symbol_strategy_map[vt_symbol] = list(set(strategies))
        ...
        for vt_symbol in strategy.vt_symbols:
            if hasattr(self.main_engine, 'BuiltInFunction'):  # 跳过内嵌式策略(BuiltInFunction)初始化
    ...
    def remove_strategy(self, strategy_name: str):
        ...
        try:
            account_id = self.get_account_id()
            PortfolioRamValue().cancel_data(strategy_name, account_id)
        except Exception as e:
            self.write_log(f"策略{strategy.strategy_name}数据库变量移除失败，请手动移除,{e}")
        return True
    ...
    def get_strategy_class_parameters_chinese(self, class_name: str):
        strategy_class = self.classes[class_name]
        return strategy_class.parameters_chinese
    ...
    def get_strategy_parameters_chinese(self, strategy_name):
        strategy = self.strategies[strategy_name]
        return strategy.get_paramenters_chinese()
    ...
    def load_strategy_setting(self):
        ...
        for strategy_name, strategy_config in strategy_setting.items():
            if strategy_config["class_name"] == "BuiltinStrategy":  # 跳过加载内嵌式策略
                continue
            self.add_strategy(
    ...
    def on_close(self, order_data, tick):
        ...
        强制平仓一条数据
        """
        strategy = self.strategies.get(order_data.strategy_name)
        if strategy:
            strategy.on_close(order_data, tick)

    def on_open_builtin(self, open_dict):
        """
        内嵌策略的开仓挂单
        """
        strategy = self.strategies.get(BISTRATEGYNAME)
        if strategy:
            strategy.on_open(open_dict)
        else:
            self.write_log('未获取到内嵌策略, 无法挂单')

    def on_cancel_builtin(self, order_id):
        """
        内嵌策略委托订单撤销
        """
        strategy = self.strategies.get(BISTRATEGYNAME)
        if strategy:
            strategy.on_cancel(order_id)
        else:
            self.write_log('未获取到内嵌策略， 无法撤单')
