...
class ReconnectMySQLDatabase(ReconnectMixin, PooledMySQLDatabase):
    _instance = None
    @staticmethod
    def get_db_instance(settings: dict):
        if not ReconnectMySQLDatabase._instance:
            ReconnectMySQLDatabase._instance = ReconnectMySQLDatabase(**settings)
...
def init(driver: Driver, settings: dict):
    ...
    bar, tick, stock_bar = init_models(db, driver)
    return SqlManager(bar, tick, stock_bar, db)
...
def init_mysql(settings: dict):
    ...
    settings['max_connections'] = '3000'
    settings['stale_timeout'] = '5'
    settings['timeout'] = '5'
    db = ReconnectMySQLDatabase.get_db_instance(settings)
    return db
...
def init_models(db: Database, driver: Driver):
    ...
    class StockDbBarData(ModelBase):
        """
        Stock K-chart historical data Mode
        """
        ...
        @staticmethod
        def save_all(objs: List["StockDbBarData"]):
            """
            save a list of objects, update if exists.
            """
            dicts = [i.to_dict() for i in objs]
            with db.atomic():
                if driver is Driver.POSTGRESQL:
                    for bar in dicts:
                        StockDbBarData.insert(bar).on_conflict(
                            update=bar,
                            conflict_target=(
                                StockDbBarData.symbol,
                                StockDbBarData.exchange,
                                StockDbBarData.interval,
                                StockDbBarData.datetime,
                            ),
                        ).execute()
                else:
                    for c in chunked(dicts, 50):
                        StockDbBarData.insert_many(
                            c).on_conflict_replace().execute()

    db.connect()
    db.create_tables([DbBarData, DbTickData, StockDbBarData])

    return DbBarData, DbTickData, StockDbBarData


class SqlManager(BaseDatabaseManager):

    def __init__(self, class_bar: Type[Model], class_tick: Type[Model], stock_class__bar: Type[Model],db):
        self.class_bar = class_bar
        self.class_tick = class_tick
        self.stock_class__bar = stock_class__bar
        self.db = db
    ...
    def load_bar_data(
        ...
        select_table = self.class_bar
        # 通过交易所代码来区分到底是股票还是期货
        if exchange.value in ["XSHE", "XSHG"]:
            select_table = self.stock_class__bar

        s = (
            select_table.select().where(
                (select_table.symbol == symbol)
                & (select_table.exchange == exchange.value)
                & (select_table.interval == interval.value)
                & (select_table.datetime >= start)
                & (select_table.datetime <= end)
            )
            .order_by(select_table.datetime)
        )
