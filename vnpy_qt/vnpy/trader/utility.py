...
def get_mac_address():
    ...
    if platform.system() == "Windows":
        ...
        for line in os.popen("/sbin/ifconfig"):
            if 'Ether' in line:
                mac_ori = line.split()[1]
                mac = "".join(mac_ori.split(':'))
                break
        return mac


BISTRATEGYNAME = 'BI' + get_mac_address()
...
def get_letter_from_symbol(symbol: str) -> str:
    """返回期货代码字母部分,并且转大写"""
    if symbol[-4:].isdigit():
        letter = symbol[:-4].upper()
    else:
        letter = symbol[:-3].upper()
    return letter


def get_contrast_direction(direction):
    """多空反转"""
...
def change_priceline(price, price_tick, direction, offset):
    """将价格取整操作"""
    if direction == Direction.LONG:
        if offset == Offset.OPEN:
            return round(price * (1 / price_tick)) * price_tick if round(
                price * (1 / price_tick)) * price_tick >= price else round(
                price * (1 / price_tick)) * price_tick + price_tick
        else:
            return round(price * (1 / price_tick)) * price_tick if round(
                price * (1 / price_tick)) * price_tick <= price else round(
                price * (1 / price_tick)) * price_tick - price_tick
...
def save_parameter_optimization_data_to_csv(result_values, target_display, strategy_name=None, symbol=None, path=None):
    """
    save the parameter optimization data to a CSV file
    """
    list2 = []
    for data in result_values:
        list1 = []
        list1.append(data[0])
        list1.append(f'{data[1]:,.2f}')
        list1.append(f'{data[2]["annual_return"]:,.2f}')
        list1.append(f'{data[2]["sharpe_ratio"]:,.2f}')
        list1.append(f'{data[2]["win_ratio"]}')
        list1.append(f'{data[2]["max_ddpercent"]:,.2f}')
        list1.append(f'{data[2]["win_loss_ratio"]}')
        list1.append(f'{data[2]["return_risk_ratio"]:,.2f}')
        list1.append(data[2]["total_trade_count"])
        list1.append(data[2]["real_yields"])
        list2.append(list1)
    df = pd.DataFrame(data=list2,
                      columns=["参数", target_display, "年化收益率", "夏普比率", "胜率", "最大回撤比", "盈亏比", "收益风险比", "总成交笔数", "实际收益率"])
    if not path:
        out_name = (symbol + '_' + strategy_name + '_' + str(datetime.now().date()))
        if platform.system() == 'Windows':
            disk_list = []
            ...
            path = os.path.join(disk_list[1], "/parameter_optimization/")
            ...
            try:
                if not os.path.exists('/home/vnpy/parameter_optimization'):
                    os.makedirs('/home/vnpy/parameter_optimization')
                df.to_csv('/home/vnpy/parameter_optimization/' + out_name + '.csv', encoding='gbk', index=False)
            finally:
                pass
    else:
        df.to_csv(path, encoding='gbk', index=False)
