...
from .dominant_contract_widget import DominantContractTradingWidget
from vnpy.trader.ui.widget import StrategyWhetherContinueOpenMonitor, ForcedLiquidationPosition
...
from .widget import (
    ...
    TradingWidget,
    AboutDialog,
    GlobalDialog, StrategyHoldPositionMonitor, StrategyClosePositionMonitor,
    StrategyLogMonitor
)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        self.widget_class = {}
        self.window_title: str = f"聚沙科技量化交易系统 {vnpy.__version__} [{TRADER_DIR}]"
    ...
    def init_dock(self) -> None:
        ...
        strategy_log_widget, strategy_log_dock = self.create_dock(
            StrategyLogMonitor, "策略日志", QtCore.Qt.TopDockWidgetArea
        )

        strategy_holding_widget, strategy_holding_dock = self.create_dock(
            StrategyWhetherContinueOpenMonitor, "策略是否持续开仓", QtCore.Qt.BottomDockWidgetArea
        )
        strategy_hold_position_widget, strategy_hold_position_dock = self.create_dock(
            StrategyHoldPositionMonitor, "策略持仓", QtCore.Qt.BottomDockWidgetArea
        )
        strategy_close_position_widget, strategy_close_position_dock = self.create_dock(
            StrategyClosePositionMonitor, "策略平仓", QtCore.Qt.BottomDockWidgetArea
        )

        new_trading_widget, new_trading_dock = self.create_dock(
            DominantContractTradingWidget, "主力合约交易", QtCore.Qt.BottomDockWidgetArea
        )
    ...
    def init_menu(self) -> None:
        ...
        for app in all_apps:
            ...
            self.widget_class[app.app_name] = widget_class = getattr(ui_module, app.widget_name)
        ...
        self.add_menu_action(
            app_menu,
            "强制平仓",
            "forced_liquidation.ico",
            partial(self.open_widget, ForcedLiquidationPosition, "forced_liquidation"),
        )
    ...
    def add_toolbar_action(
        ...
        # if action_name in ["强制平仓"]:  # func <=> open_widget(),程序一启动就打开强制平仓部件。
        #     func()
    ...
    # 获取模块类
    def get_widget_class(self, app_name):
        if app_name in self.widget_class:
            return self.widget_class[app_name]
        else:
            return None
    ...
    def open_widget(self, widget_class: QtWidgets.QWidget, name: str) -> None:
        ...
        if isinstance(widget, QtWidgets.QDialog):
            ...
            # if name == "forced_liquidation":
            #     if widget.force_table.rowCount():
            #         self.main_engine.get_engine("forced_liquidation").count = 10  # 设置TransactionPushEngine的count属性值(此时设置为10为了定时器马上就推送)
            #         widget.show()
            # else:
            widget.show()
    ...
    def send_test_email(self) -> None:
        ...
        """
        self.main_engine.send_email("聚沙科技有限公司", "量化交易框架测试")
