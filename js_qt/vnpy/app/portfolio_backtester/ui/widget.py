...
from vnpy.trader.symbol_attr import ContractMultiplier, ContractPricePick, SymbolCommissionRate
...
from ..engine import (
    APP_NAME,
    EVENT_PORTFOLIO_BACKTESTER_LOG,
    EVENT_PORTFOLIO_BACKTESTING_FINISHED, EVENT_PORTFOLIO_BACKTESTER_OPTIMIZATION_FINISHED,
)
...
class BacktesterManager(QtWidgets.QWidget):
    """"""
    setting_filename = "portfolio_backtester_setting.json"
    ...
    def init_ui(self):
        ...
        self.log_monitor.setMaximumHeight(140)

        self.chart = BacktesterChart()
        self.chart.setMinimumWidth(1050)
    ...
    def register_event(self):
        ...
        self.event_engine.register(EVENT_PORTFOLIO_BACKTESTER_LOG, self.signal_log.emit)
        self.event_engine.register(
            EVENT_PORTFOLIO_BACKTESTING_FINISHED, self.signal_backtesting_finished.emit)

        self.event_engine.register(EVENT_PORTFOLIO_BACKTESTER_OPTIMIZATION_FINISHED, self.process_optimization_finished_event)
    ...
    def process_backtesting_finished_event(self, event: Event):
        ...
        self.statistics_monitor.set_data(statistics)  # 往指标统计监测中设置数据
    ...
    def start_backtesting(self):
        ...
        if vt_symbol[-1] == ",":
            vt_symbol = vt_symbol[:-1]
        vt_symbol_list = []
        rates = {}
        slippages = {}
        sizes = {}
        priceticks = {}
        for vt_symbol in vt_symbol.split(","):
            vt_symbol = vt_symbol.strip()
            vt_symbol_list.append(vt_symbol)
            symbol_letter = get_letter_from_symbol(vt_symbol.split(".")[0])
            rates[vt_symbol] = SymbolCommissionRate[symbol_letter]
            slippages[vt_symbol] = float(self.slippage_line.text())
            sizes[vt_symbol] = ContractMultiplier[symbol_letter]
            priceticks[vt_symbol] = ContractPricePick[symbol_letter]
    ...
    def show_daily_results(self):
        ...
        if not self.daily_dialog.is_updated():
            ...
            # print(min(end_pos_list), max(end_pos_list))
...
class BacktesterChart(pg.GraphicsWindow):
    ...
    def init_ui(self):
        """"""
        pg.setConfigOptions(antialias=True)  # 设置全局配置选项。
...
class DailyResultMonitor(BaseMonitor):
    ...
    headers = {
        ...
        # "start_pos": {"display": "开盘持仓", "cell": BaseCell, "update": False},  # 暂时未计算开盘持仓
