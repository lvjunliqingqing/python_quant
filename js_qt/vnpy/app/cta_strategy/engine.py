...
from ..utility import get_message_box
...
class CtaEngine(BaseEngine):
    ...
    def init_engine(self):
        """"""
        if not SETTINGS['rqdata.username']:
            self.write_log("未使用米筐,改用本地数据库初始化")
        else:
            try:
                self.init_rqdata()
            except Exception as e:
                self.write_log("初始化米筐异常,异常信息为:", e)
    ...
    def send_order(
        ...
        if stop:
            if contract.stop_supported:
                return self.send_server_stop_order(strategy, contract, direction, offset, price, volume, lock, order_ref)
            else:
                return self.send_local_stop_order(strategy, direction, offset, price, volume, lock, order_ref)
        else:
            return self.send_limit_order(strategy, contract, direction, offset, price, volume, lock, order_ref)
    ...
    def add_strategy(
        ...
        self.save_new_created_strategy(vt_symbol, strategy_name, strategy, class_name)
        self.strategies[strategy_name] = strategy
    ...
    def _init_strategy(self, strategy_name: str):
        ...
        if contract:
            ...
            self.write_log(f"{strategy.vt_symbol}行情订阅失败，{strategy_name}初始化失败,请确保能订阅行情后再来初始化", strategy)
    ...
    def remove_strategy(self, strategy_name: str):
        ...
        try:
            ...
            RamValue().cancel_data(strategy_name, account_id)
        except Exception:
            self.write_log(f"策略{strategy.strategy_name}数据库变量移除失败，请手动移除")
        return True
    ...
    def save_new_created_strategy(self, vt_symbol, strategy_name, strategy, class_name):
        """Save the created policy to the database"""
        symbol, exchange = extract_vt_symbol(vt_symbol)
        account_id = self.get_account_id()
        ramValue = RamValue()
        value = ramValue.search_value(strategy_name, account_id)
        if not value:
            strategy_value = strategy.strategy_value
            sql = ramValue.insert_value(class_name, symbol, strategy_name, strategy_value, account_id)
            if not sql:
                self.write_log(f"创建策略失败，插入数据库失败")
                return
        else:
