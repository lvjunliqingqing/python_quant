...
class CtaManager(QtWidgets.QWidget):
    ...
    def add_strategy(self):
        ...
        parameters_chinese = self.cta_engine.get_strategy_class_parameters_chinese(class_name)
        editor = SettingEditor(parameters, class_name=class_name, parameters_chinese=parameters_chinese)
        n = editor.exec_()
        ...
        if n == editor.Accepted:
            ...
            frist_strategy_name = strategy_name

            if ';' in vt_symbol:
                """"""
                symbol_tmp = vt_symbol.split(';')
                for _symbol in symbol_tmp:
                    if _symbol:
                        strategy_name = frist_strategy_name + "_" +_symbol
                        self.cta_engine.add_strategy(
                            class_name, strategy_name, _symbol, setting
                        )
            else:
                self.cta_engine.add_strategy(
    ...
    def init_all_strategies(self):
        if not self.main_engine.connection_flag:
            message_box = get_message_box(title="全部初始化警告", x_offset=180, text="请确认登录账户成功后，再进行全部初始化")
            message_box.exec()
            return

        if not self.main_engine.get_all_contracts():
            message_box = get_message_box(title="全部初始化警告", x_offset=180, text="请确认能获行情取数据后，再进行全部初始化")
            message_box.exec()
            return
...
class StrategyManager(QtWidgets.QFrame):
    ...
    def init_strategy(self):
        """"""
        if not self.cta_engine.main_engine.connection_flag:
            message_box = get_message_box(title="初始化警告", text="请确认登录账户后，再进行初始化")
            message_box.exec()
            return

        strategy = self.cta_engine.strategies[self.strategy_name]
        if not self.cta_engine.main_engine.get_contract(strategy.vt_symbol):
            message_box = get_message_box(title="初始化警告", x_offset=180, text=f"请确认能获{strategy.vt_symbol}合约数据后，再进行初始化")
            message_box.exec()
            return
    ...
    def edit_strategy(self):
        ...
        parameters_chinese = self.cta_engine.get_strategy_parameters_chinese(strategy_name)
        editor = SettingEditor(parameters, strategy_name=strategy_name, parameters_chinese=parameters_chinese)
        n = editor.exec_()
...
class SettingEditor(QtWidgets.QDialog):
    ...
    def __init__(
        self, parameters: dict, strategy_name: str = "", class_name: str = "", parameters_chinese: dict = {}
    ):
        """"""
        ...
        self.parameters_chinese = parameters_chinese
    ...
    def init_ui(self):
        ...
        if self.class_name:
            ...
            parameters_chinese = {"strategy_name": "strategy_name", "vt_symbol": "vt_symbol"}
            parameters_chinese.update(self.parameters_chinese)
        else:
            self.setWindowTitle(f"参数编辑：{self.strategy_name}")
            button_text = "确定"
            parameters = self.parameters
            parameters_chinese = self.parameters_chinese
        ...
        for name, value in parameters.items():
            ...
            if type_ is int:
                ...
                edit.setValidator(validator)
            if self.parameters_chinese:
                form.addRow(f"{parameters_chinese[name]} {type_}", edit)
            else:
