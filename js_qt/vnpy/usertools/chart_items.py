...
class SmaItem(CandleItem):
    ...
    def get_sma_value(self, ix: int) -> float:
        ...
        if not self.sma_data:
            ...
            # 缺失值替换成第一非nan的元素
            sma_array[np.isnan(sma_array)] = sma_array[~np.isnan(sma_array)][0]
...
class RsiItem(ChartItem):
    ...
    def __init__(self, manager: BarManager):
        """"""
        super().__init__(manager)
        self.white_pen: QtGui.QPen = pg.mkPen(color=(255, 51, 51), width=1)
    ...
    def get_rsi_value(self, ix: int) -> float:
        ...
        if not self.rsi_data:
            ...
            # 缺失值替换成第一非nan的元素
            rsi_array[np.isnan(rsi_array)] = rsi_array[~np.isnan(rsi_array)][0]
    ...
    def _draw_bar_picture(self, ix: int, bar: BarData) -> QtGui.QPicture:
        ...
        if not np.isnan(last_rsi_value) and not np.isnan(rsi_value):
            end_point = QtCore.QPointF(ix, rsi_value)
            start_point = QtCore.QPointF(ix - 1, last_rsi_value)
            painter.drawLine(start_point, end_point)

        self.white_pen.setStyle(Qt.DashLine)  # 线虚线
        painter.setPen(self.white_pen)
        # 画超卖、超买线
        painter.drawLine(
...
class MacdItem(ChartItem):
    ...
    def get_macd_value(self, ix: int) -> Tuple[float, float, float]:
        ...
        if not self.macd_data:
            ...
            diffs, deas, macds = talib.MACD(np.array(close_data),
                                            ...
                                            signalperiod=self.M)
            # 缺失值替换成第一非nan的元素
            diffs[np.isnan(diffs)] = diffs[~np.isnan(diffs)][0]
            deas[np.isnan(deas)] = deas[~np.isnan(deas)][0]
            macds[np.isnan(macds)] = macds[~np.isnan(macds)][0]
    ...
    def _draw_bar_picture(self, ix: int, bar: BarData) -> QtGui.QPicture:
        ...
        if np.isnan(macd_value[1]) or np.isnan(last_macd_value[1]):
            ...
            painter.setPen(self.yellow_pen)
            painter.drawLine(start_point1, end_point1)  # 绘制线
        ...
        if not np.isnan(macd_value[2]):
            ...
            painter.drawRect(QtCore.QRectF(ix - 0.3, 0, 0.6, macd_value[2]))  # 绘制矩形框
        else:
            # print("略过macd lines2")
            pass
    ...
    def get_info_text(self, ix: int) -> str:
        ...
        if ix in self.macd_data:
            ...
            text = f"MACD({self.short_window},{self.long_window},{self.M})  \nMACD - \nDIFF - \nDEA -"
    ...
    def get_info_text(self, ix: int) -> str:
        ...
        if ix in self.trades:
            ...
            for tradeid, trade in self.trades[ix].items():
                text += f"\n价格:{trade.price} 方向:{trade.direction.value}{trade.offset.value} 成交量:{trade.volume}手"
        else:
            text = "成交单：-"
        return text
    ...
    def get_info_text(self, ix: int) -> str:
        ...
        if ix in self.orders:
            ...
            for orderid, order in self.orders[ix].items():
                text += f"\n价格:{order.price} 方向:{order.direction.value}{order.offset.value} 成交量:{order.volume}手"
        else:
            text = "委托单:-"
