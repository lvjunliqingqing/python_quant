...
class FuturesIndex:
    ...
    def read_futures_price_daily(self):
        """期货日K线数据表"""
        security = ('ER9999.XZCE', 'ME9999.XZCE', 'PK9999.XZCE', 'RO9999.XZCE', 'TC9999.XZCE', 'WS9999.XZCE', 'WT9999.XZCE')
        ...
        # sql = f'SELECT security,trade_date,open,close,low,high FROM dwd_futures_price_daily WHERE security in {security} and (trade_date between "{self.start_date}" and "{self.end_date}") and adjust="none"'  # 正则匹配
        ...
        sql = f'SELECT security,trade_date,open,close,low,high,volume,open_interest,pre_close,high_limit,change_pct FROM dwd_futures_price_daily WHERE security REGEXP ".*(9999).*" ' \
              f'and security not in {security} and (trade_date between "{self.start_date}" and "{self.end_date}") and adjust="none" ORDER BY trade_date'  # 正则匹配

        df = self.read_mysql(sql)
        if not df.empty:
            df = df[~((df["low"] == df["high"]) & (df["volume"] == 0))]  # 过滤掉停盘数据
            df["trade_date"] = df["trade_date"].apply(lambda x: x.strftime("%Y-%m-%d"))
            df["up_limit"] = round((df["high_limit"] - df["pre_close"]) / df["pre_close"], 3) * 100  # 计算当天的涨跌幅限制
        return df
    ...
    def read_dwd_dominant_futures_winners_list(self, security, member_name):
    ...
    def read_futures_main_force_continuous(self, today_date):
        ...
        df = self.read_mysql(sql)
        return df["security"].to_list()

    def read_futures_main_price_daily(self, security):
        sql = f'SELECT * FROM dwd_futures_price_daily WHERE security="{security}" and adjust="post"'
        df = self.read_mysql(sql)
...
if __name__ == '__main__':
    ...
    # futures_price_df = futures_Index.read_futures_bias_daily()
    # futures_price_df = futures_Index.read_futures_rsi()
    # futures_price_df = futures_Index.read_futures_boll()
    # futures_price_df = futures_Index.read_futures_macd()
    ...
    futures_price_df = futures_Index.read_futures_contract_info()
    # futures_price_df = futures_Index.read_futures_base_info()
