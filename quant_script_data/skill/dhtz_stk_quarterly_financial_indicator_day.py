...
class FinancialIndicatorDayModel(Model):
    ...
    class Meta:
        database = db
        table_name = 'dhtz_stk_quarterly_financial_indicator_day'
    ...
    def batch_sava_financial_indicator_day_data(self, df):
        data = []
        for idx, row in df.iterrows():
            data.append(
                {
            ...
            with db.atomic():
                sql = FinancialIndicatorDayModel.insert_many(data)\
                    .on_conflict(
                        preserve=[
                            FinancialIndicatorDayModel.code,
                            FinancialIndicatorDayModel.pubDate,
    ...
    def get_financial_indicators_table_data(self):
        ...
        q = query(
            ...
            indicator.code.in_(symbol_list)
        )
        for i in range(2010, 2021):
            for j in range(1, 5):
                df = get_fundamentals(q, statDate=f"{i}q{j}")
                df.dropna(axis=0, how='all', inplace=True)
                if not len(df):
                    print(f"{i}q{j} the data for:{len(df)}")
                else:
                    ...
                    # df.to_csv("./roe_test.csv", encoding="utf-8", index=False, columns=columns)
                    with db.atomic():
                        self.sava_financial_indicator_day_data(df)
                        # self.batch_sava_financial_indicator_day_data(df)
                    time.sleep(1)
